...
from utils import *
from testing_tables.test_tables import table_sport_a, table_sport_b, table_A, table_B, table_stocks_a, table_stocks_b


@timeout_decorator.timeout(var.timeout_s)
def detect_seeds(r_tab, s_tab, r_w, s_w, min_h, row_constraint, col_constraint):
    """
...
def sloth_c(r_tab, s_tab, min_a=0, min_w=0, max_w=math.inf, min_h=0, max_h=math.inf, bw=var.default_bw, complete=False,
    ...
    try:
        seeds, top_lev = detect_seeds(r_tab, s_tab, r_w, s_w, min_h, row_constraint, col_constraint)
        num_seeds = len(seeds)
    ...
    if len(results) > 0:
        reconstructed = reconstruct_largest_overlap(r_tab, s_tab, seeds, results, row_constraint)
        return reconstructed, metrics

    return results, metrics


def reconstruct_largest_overlap(r_tab, s_tab, seeds, results, row_constraint='none'):
    """
    Ricostruisce il largest overlap tra le tabelle R e S dato il risultato dell'algoritmo.
    
    Parameters:
    -----------
    r_tab : list of lists
        Tabella R in formato lista di colonne
    s_tab : list of lists
        Tabella S in formato lista di colonne
    seeds : list of tuples
        Lista dei seeds trovati dall'algoritmo, ogni seed è una tupla ((x,y), mapping, height)
    results : list
        Lista dei risultati dell'algoritmo (indices dei seeds che formano il largest overlap)
    row_constraint : str, default='none'
        Vincolo sulle righe: 'none', 'ordered', o 'contiguous'
        
    Returns:
    --------
    df : pandas.DataFrame o list
        Per row_constraint='none': ritorna la lista originale di results trasformata
        Per altri vincoli: DataFrame contenente il largest overlap ricostruito
    """
    # Estrae le coppie di colonne corrispondenti dal risultato migliore (results[0])
    col_pairs = [seeds[s_id][0] for s_id in results[0]]
    # Ordina le coppie per indice crescente della prima colonna
    col_pairs.sort(key=lambda s: s[0], reverse=False)
    # Crea una sottotabella prendendo solo le colonne di R coinvolte nel mapping
    common_subtable = [r_tab[m[0]] for m in col_pairs]
    # Crea un dizionario che mappa gli indici delle colonne ai loro valori
    dati_dict = {idx[0]: valori for idx, valori in zip(col_pairs, common_subtable)}

    # Gestisce i tre diversi vincoli sulle righe:
    if row_constraint == 'ordered':
        # trova la più lunga sottosequenza comune (LCS)
        _, idx_list = compute_long_comm_subseq(r_tab, s_tab, results[0], seeds, index=True)
        # Crea DataFrame usando solo le righe coinvolte nella LCS
        df = pd.DataFrame(dati_dict).iloc[idx_list]

    else: 
        # trova la più lunga sottostringa comune contigua
        h, start_idx = compute_long_comm_substr(r_tab, s_tab, results[0], seeds, index=True)
        end_idx = start_idx + h - 1
        # Crea DataFrame usando il range contiguo di righe trovato
        df = pd.DataFrame(dati_dict).loc[start_idx:end_idx]

    return df

# r_tab = pd.read_csv('/Users/davidelupo/Desktop/sloth-main/Dataset/wikitables/test/tables/105.41841.csv')
# r_tab_col_list = parse_table_from_dataframe(r_tab)
# s_tab = pd.read_csv('/Users/davidelupo/Desktop/sloth-main/Dataset/wikitables/test/tables/562.74191.csv')
# s_tab_col_list = parse_table_from_dataframe(s_tab)

# r_tab = pd.read_csv('testing_tables/reverse_cols#1.csv', sep=';')
# r_tab_col_list = parse_table_from_dataframe(r_tab)
# s_tab = pd.read_csv('testing_tables/reverse_cols#2.csv', sep=';')
# s_tab_col_list = parse_table_from_dataframe(s_tab)

# row_c, col_c = input_from_console()
# results, metrics = sloth_c(r_tab_col_list, s_tab_col_list, verbose=True, metrics=[], row_constraint=row_c, col_constraint=col_c)
# print(metrics)
# print('num_seeds:', metrics[0])
# print('algo:', metrics[2])
# print('w:', metrics[10])
# print('h:', metrics[11])
# print('overlap:', metrics[12])
# print('time:', metrics[13])

path_dataset = '/Users/davidelupo/Desktop/sloth-main/Dataset'
n_samples = 1000  # Number of samples to process
dataset = 'git'
constraint = 'ordered'  # 'none', 'ordered', 'contiguous'
...
for idx in range(len(sloth_csv)):
    ...
    # avg_dup = analisi_duplicati(r_tab, s_tab)
    # if avg_dup > threshold_dup:
    #     print()
    #     print(f'Skipping pair {sloth_csv["r_id"][idx]} and {sloth_csv["s_id"][idx]} due to high duplication ratio: {avg_dup:.2f}')
    #     print()
    #     count_skipped += 1
    #     continue
    ...
    # duplicates_r_tab = False
    # for col in r_tab_col_list:
    #     if len(set(col)) != r_h:
    #         duplicates_r_tab = True
    #         break

    # duplicates_s_tab = False
    # for col in s_tab_col_list:
    #     if len(set(col)) != s_h:
    #         duplicates_s_tab = True
    #         break
    
    # if duplicates_r_tab and duplicates_s_tab:
    #     continue

    results, metrics = sloth_c(r_tab_col_list, s_tab_col_list, verbose=True, metrics=[], row_constraint=constraint, col_constraint=constraint)
...
df_results = pd.DataFrame({'r_id': r_id_list, 's_id': s_id_list, 
                           ...
                           'ver_cands': num_ver_cands, 'ver_time': ver_cands_time})
df_results.to_csv(f'final_results_{dataset}/test_sloth_{constraint}_{dataset}_{n_samples}samples.csv', sep=';', index=False)

# print('Number of skipped pairs due to high duplication ratio:', count_skipped)