...
import numpy as np
from utils import *
from testing_tables import test_tables


@timeout_decorator.timeout(var.timeout_s)
def detect_seeds(r_tab, s_tab, r_w, s_w, min_h, row_constraint=None):
    ...
    for x in range(0, r_w):
        for y in range(0, s_w):
            ...
            # h_true = LCSseq.similarity(r_tab[x], s_tab[y])
            # h_true,_,_ = longest_common_substring(r_tab[x], s_tab[y])
            # if h_true != h:
            #     print('Bug') 
            #     print('h_true:', h_true, 'h:', h)
...
def sloth_mapping(r_tab, s_tab, min_a=0, min_w=0, max_w=math.inf, min_h=0, max_h=math.inf, bw=var.default_bw, complete=False,
    ...
    try:
        seeds, top_lev = detect_seeds(r_tab, s_tab, r_w, s_w, min_h, row_constraint=row_constraint)
        num_seeds = len(seeds)
    ...
    # Reconstruct the largest common subtable
    # common_subtable = [r_tab[m[0]] for m in col_pairs]

    # dati_dict = {idx[0]: valori for idx, valori in zip(col_pairs, common_subtable)}

    # col_pairs = map(lambda i: set(seeds[i][3]), results[0])
    # intersect = set.intersection(*col_pairs)
    # intersect = sorted(intersect, key=lambda pair: pair[0])

    # idx_list = lis_bisect([pair[1] for pair in intersect])
    # c_h = lis_length_only([pair[1] for pair in intersect])

    # rows_r = len(set(i for i, j in intersect))
    # c_h = min(c_h, rows_r)
...
# r_tab = pd.read_csv('/Users/davidelupo/Desktop/sloth-main/Dataset/gittables/test/tables/object_csv_licensed.zip_IA2ComProxy.dll.csv__2060878506__.csv',header=None)
# r_tab_col_list = parse_table_from_dataframe(r_tab)
# s_tab = pd.read_csv('/Users/davidelupo/Desktop/sloth-main/Dataset/gittables/test/tables/object_csv_licensed.zip_bcastdvr.proxy.dll.csv__2150601478__.csv',header=None)
# s_tab_col_list = parse_table_from_dataframe(s_tab)

# r_tab = pd.read_csv('testing_tables/flightA.csv', sep=';')
# r_tab_col_list = parse_table_from_dataframe(r_tab)
# s_tab = pd.read_csv('testing_tables/flightB.csv', sep=';')
# s_tab_col_list = parse_table_from_dataframe(s_tab)
# result = estimate_mapping_density(r_tab, s_tab, threshold_dup=0.4)
# print(result)
...
for idx in range(len(sloth_csv)):
    ...
    if R > threshold_dup:
        print()
        print(f'Skipping pair {sloth_csv["r_id"][idx]} and {sloth_csv["s_id"][idx]} due to high mapping density ratio: {R:.2f}')
        print()
        continue

    results, metrics = sloth_mapping(r_tab_col_list, s_tab_col_list, verbose=True, metrics=[], row_constraint=constraint, col_constraint=constraint)
    idx_list.append(idx)
...
df_results = pd.DataFrame({'r_id': r_id_list, 's_id': s_id_list, 
                           ...
                           'ver_cands': num_ver_cands, 'ver_time': ver_cands_time, 'idx': idx_list})
df_results.to_csv(f'final_results_{dataset}/test_mapping_{constraint}_{dataset}_{n_samples}samples.csv', sep=';', index=False)

# df_results = pd.DataFrame({'r_id': r_id_list, 's_id': s_id_list})
# dup_results_df = pd.DataFrame(dup_results)

# merged_df = pd.concat([df_results.reset_index(drop=True),
#                        dup_results_df.reset_index(drop=True)], axis=1)

# merged_df.to_csv(f'final_results_{dataset}/dup_analysis_{dataset}_{n_samples}samples.csv', sep=';', index=False)