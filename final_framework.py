...
def detect_seeds_mapping(r_tab, s_tab, r_w, s_w, min_h, row_constraint=None):
    ...
    for x in range(0, r_w):
        for y in range(0, s_w):
            list_mapping = find_matches_between_columns(r_tab[x], s_tab[y])
            if row_constraint == 'ordered':
                h = longest_increasing_pairs(list_mapping)
            elif row_constraint == 'contiguous':
                h = longest_consecutive_pairs(list_mapping)
            else:
                first_indices = {pair[0] for pair in list_mapping}
                second_indices = {pair[1] for pair in list_mapping}
                h = min(len(first_indices), len(second_indices))
            if h > max(0, min_h - 1):
                ...
                seeds.append((s, m, h, list_mapping))  # the seed is register as a tuple of 3 elements: ((x, y), mapping, height)
...
def approximate_algorithm_mapping(bw, r_tab, s_tab, seeds, num_seeds, top_lev, theta, min_w, max_w, min_h, max_h, results,
                          res_h, complete, verbose, metrics, row_constraint=None, col_constraint=None):
    """
        ...
        while len(probes) > 0 and lev_w <= top_lev:
            ...
            for p in probes:
                ...
                for i in p_c:
                    ...
                    col_pairs = [seeds[i][0] for i in c_s]
                    col_pairs.sort(key=lambda s: s[0], reverse=False)

                    first_elements = [x[0] for x in col_pairs]
                    second_elements = [x[1] for x in col_pairs]

                    if col_constraint == 'ordered':
                        # Check if both sequences are strictly increasing
                        first_increasing = all(first_elements[i] < first_elements[i + 1] for i in range(len(first_elements) - 1))
                        second_increasing = all(second_elements[i] < second_elements[i + 1] for i in range(len(second_elements) - 1))
                        valid = first_increasing and second_increasing
                        
                    if col_constraint == 'contiguous':
                        # Check if both sequences are strictly increasing and consecutives
                        first_increasing = all(first_elements[i + 1] - first_elements[i] != 1 for i in range(len(first_elements) - 1))
                        second_increasing = all(second_elements[i + 1] - second_elements[i] != 1 for i in range(len(second_elements) - 1))
                        valid = first_increasing and second_increasing

                    # Check that the seed is compatible
                    if (p_m & seeds[i][1]).count(True) > 0 or not(valid):
                        # print('INVALID',i,'\n')
                        s_inc.add(i)
                        ...
                        if c_hash not in lev_cache.keys():
                            ...
                            # Estrai solo gli insiemi di coppie dai seed indicati in top_c[0]
                            col_pairs = map(lambda i: set(seeds[i][3]), tuple(c_s))
                            # Intersezione diretta
                            intersect = set.intersection(*col_pairs)
                            # Ordina per primo elemento della tupla (pair[0])
                            intersect = sorted(intersect, key=lambda pair: pair[0])

                            if row_constraint == 'none':
                                c_h = sum(compute_bag_intersection(r_tab, s_tab, tuple(c_s), seeds).values())
                            elif row_constraint == 'ordered':
                                c_h = longest_increasing_pairs(intersect)
                            else:
                                c_h = longest_consecutive_pairs(intersect)
...
def exact_algorithm_mapping_setup(seeds, top_lev, theta, min_w, max_w):
    """
    ...
    gen_pq.sort(key=lambda lev: (lev[2], lev[0]), reverse=True) #ordino prima per lev_max_a decrescente, e a parità di valore, per lev_w decrescente.
    return gen_pq


@timeout_decorator.timeout(var.timeout_e)
def exact_algorithm_mapping(r_tab, s_tab, r_w, s_w, seeds, num_seeds, top_lev, theta, min_w, max_w, min_h, max_h, results,
                    res_h, complete, verbose, metrics, row_constraint=None, col_constraint=None):
    """
    ...
    gen_pq = exact_algorithm_mapping_setup(seeds, top_lev, theta, min_w, max_w)
    cand_pq = list()
    ...
    while len(cand_pq) > 0 or len(gen_pq) > 0:
        ...
        while len(gen_pq) > 0:
            ...
            # controlla che ci siano abbastanza elementi in entrambe le parti della maschera per creare una combinazione valida di larghezza c_w
            if min(c_w, m.count(True, 0, r_w), m.count(True, r_w)) >= c_w:
                ...
                for nc in new_cands:
                    ...
                    if c_m.count(True, 0, r_w) == c_m.count(True, r_w) == c_w:
                        col_pairs = [seeds[i][0] for i in nc]
                        col_pairs.sort(key=lambda s: s[0], reverse=False)
        ...
        if len(cand_pq) > 0:
            ...
            if top_c[5]:
                ...
                col_pairs = map(lambda i: set(seeds[i][3]), top_c[0])
                # Intersezione diretta
                intersect = set.intersection(*col_pairs)
                # Ordina per secondo elemento della tupla (pair[1])
                intersect = sorted(intersect, key=lambda pair: pair[0])
...
def sloth_mapping_based(r_tab, s_tab, min_a=0, min_w=0, max_w=math.inf, min_h=0, max_h=math.inf, bw=var.default_bw, complete=False,
          verbose=True, metrics=None, row_constraint=None, col_constraint=None):
    ...
    print(f'Table R num_rows: {r_h} and num_cols: {r_w} ')
    print(f'Table S num_rows: {s_h} and num_cols: {s_w}')
    ...
    try:
        seeds, top_lev = detect_seeds_mapping(r_tab, s_tab, r_w, s_w, min_h, row_constraint=row_constraint)
        num_seeds = len(seeds)
    ...
    try:
        results, metrics = exact_algorithm_mapping(r_tab, s_tab, r_w, s_w, seeds, num_seeds, top_lev, theta, min_w, max_w,
                                           min_h, max_h, results, res_h, complete, verbose, metrics, row_constraint=row_constraint,
                                           col_constraint=col_constraint)
    except Exception as exc:
        print(exc)
        if var.run_approximate:
            try:
                results, metrics = approximate_algorithm_mapping(bw, r_tab, s_tab, seeds, num_seeds, top_lev, theta, min_w,
                                                         max_w, min_h, max_h, results, res_h, complete, verbose,
                                                         metrics, row_constraint=row_constraint,
                                                         col_constraint=col_constraint)
            except Exception as exc:
                print(exc)
                return [], metrics
...
@timeout_decorator.timeout(var.timeout_s)
def detect_seeds_lcs(r_tab, s_tab, r_w, s_w, min_h, row_constraint, col_constraint):
    """
    ...
    for x in range(0, r_w):
        for y in range(0, s_w):
            ...
            if row_constraint == 'ordered':
                h = LCSseq.similarity(r_tab[x], s_tab[y]) 
            elif row_constraint == 'contiguous':
                h, start_r, start_s = longest_common_substring(r_tab[x], s_tab[y])
            else:
                o = r_bags[x] & s_bags[y]
                h = sum(o.values())  
...
@timeout_decorator.timeout(var.timeout_a)
def approximate_algorithm_lcs(bw, r_tab, s_tab, seeds, num_seeds, top_lev, theta, min_w, max_w, min_h, max_h, results,
                          res_h, complete, verbose, metrics, row_constraint, col_constraint):
    """
    Beam-search-based approximate algorithm to detect the largest overlaps between the two tables R(X) and S(Y)
    (refer to the function detect_largest_overlaps for the parameter descriptions)
    - the seed is represented as a tuple ((col_pair), bit_array, h)
                    ...
                    if (p_m & seeds[i][1]).count(True) > 0 or not(valid):
                        ...
                        if c_hash not in lev_cache.keys():
                            ...
                            if row_constraint == 'none':
                                ...
                                c_h = compute_long_comm_subseq(r_tab, s_tab, tuple(c_s), seeds, index=False)
                            else:
                                c_h = compute_long_comm_substr(r_tab, s_tab, tuple(c_s), seeds, index=False)
...
def exact_algorithm_lcs_setup(seeds, top_lev, theta, min_w, max_w):
    """
    Initialize the generator priority queue, representing the levels of the lattice
    (refer to the function detect_largest_overlaps for the parameter descriptions)
    - the lattice level is represented as a tuple (w, seed_ptr, max_a)
...
@timeout_decorator.timeout(var.timeout_e)
def exact_algorithm_lcs(r_tab, s_tab, r_w, s_w, seeds, num_seeds, top_lev, theta, min_w, max_w, min_h, max_h, results,
                    res_h, complete, verbose, metrics, row_constraint, col_constraint):
    """
    Exact algorithm to detect the largest overlaps between the two tables R(X) and S(Y)
    (refer to the main function for the description of the parameters)
    ...
    # Setup: initialize the two priority queues (i.e., generator and candidates)
    start_setup_time = time.time()
    gen_pq = exact_algorithm_lcs_setup(seeds, top_lev, theta, min_w, max_w)
    cand_pq = list()
    setup_time = time.time() - start_setup_time
    if metrics is not None:
        metrics.append(setup_time)
        ...
        if len(cand_pq) > 0:
            ...
            if top_c[5]:
                ...
                if row_constraint == 'none':
                    ...
                    c_h = compute_long_comm_subseq(r_tab, s_tab, top_c[0], seeds, index=False)
                else:
                    c_h = compute_long_comm_substr(r_tab, s_tab, top_c[0], seeds, index=False)
...
def sloth_LCS_based(r_tab, s_tab, min_a=0, min_w=0, max_w=math.inf, min_h=0, max_h=math.inf, bw=var.default_bw, complete=False,
          verbose=True, metrics=None, row_constraint='none', col_constraint='none'):
    ...
    try:
        seeds, top_lev = detect_seeds_lcs(r_tab, s_tab, r_w, s_w, min_h, row_constraint, col_constraint)
        num_seeds = len(seeds)
    except Exception as exc:
        seeds = list()
        num_seeds = -1
    ...
    # Detect the largest overlaps
    try:
        results, metrics = exact_algorithm_lcs(r_tab, s_tab, r_w, s_w, seeds, num_seeds, top_lev, theta, min_w, max_w,
                                           min_h, max_h, results, res_h, complete, verbose, metrics, row_constraint, col_constraint)
    except Exception as exc:
        print(exc)
        if var.run_approximate:
            try:
                results, metrics = approximate_algorithm_lcs(bw, r_tab, s_tab, seeds, num_seeds, top_lev, theta, min_w,
                                                         max_w, min_h, max_h, results, res_h, complete, verbose,
                                                         metrics, row_constraint, col_constraint)
            except Exception as exc:
                print(exc)
                return [], metrics
...
# path_dataset = '/Users/davidelupo/Desktop/sloth-main/Dataset'
# n_samples = 5  # Number of samples to process
# dataset = 'wiki'
# constraint = 'ordered'  # 'ordered', 'contiguous', or 'none'
# sloth_csv = pd.read_csv(f'{path_dataset}/{dataset}tables/test.csv')[['r_id','s_id','seeds','a%','total_time']].iloc[:n_samples].reset_index(drop=True)
# ids_to_keep = set(sloth_csv['r_id']).union(set(sloth_csv['s_id']))
# tables_dict = generate_table_dict(f'{path_dataset}/{dataset}tables/test', ids_to_keep)
...
# dup_results = []
# flag_list = []
# for idx in range(len(sloth_csv)):
#     print('iteration N.',idx)
#     r_tab = tables_dict[sloth_csv['r_id'][idx]]
#     r_tab_col_list = parse_table_from_dataframe(r_tab)
#     s_tab = tables_dict[sloth_csv['s_id'][idx]]
#     s_tab_col_list = parse_table_from_dataframe(s_tab)

#     r_w = len(r_tab_col_list)
#     r_h = len(r_tab_col_list[0])
#     r_a = r_w * r_h
#     s_w = len(s_tab_col_list)
#     s_h = len(s_tab_col_list[0])
#     s_a = s_w * s_h

#     result = estimate_mapping_density(r_tab, s_tab, threshold_dup=0.2)
#     dup_results.append(result)
#     R = result['Mapping_density_ratio']
#     if R < threshold_dup:
#         flag_list.append('mapping')
#         results, metrics = sloth_mapping_based(r_tab_col_list, s_tab_col_list, verbose=True, metrics=[], row_constraint=constraint, col_constraint=constraint)
#     else:
#         results, metrics = sloth_LCS_based(r_tab_col_list, s_tab_col_list, verbose=True, metrics=[], row_constraint=constraint, col_constraint=constraint)
#         flag_list.append('LCS')
    
#     print('results:', results)
#     print('metrics:', metrics)

#     r_id_list.append(sloth_csv['r_id'][idx])
#     s_id_list.append(sloth_csv['s_id'][idx])

#     num_seeds = metrics[0]
#     if num_seeds < 1 or len(results) == 0:
#         num_seeds_list.append(num_seeds)
#         algo_list.append('e')
#         a_perc_list.append(0) 
#         time_list.append(metrics[1])
#         num_gen_cands.append(0)
#         gen_cands_time.append(0)
#         num_ver_cands.append(0)
#         ver_cands_time.append(0)
#         continue

#     max_o = metrics[12]
#     if max_o:
#         a_perc = max_o / min(r_a, s_a)
#     else:
#         a_perc = None

#     num_seeds_list.append(num_seeds)
#     seeds_time.append(metrics[1])
#     algo_list.append(metrics[2])
#     a_perc_list.append(a_perc) 
#     time_list.append(metrics[13])
#     num_gen_cands.append(metrics[5])
#     gen_cands_time.append(metrics[6])
#     num_ver_cands.append(metrics[7])
#     ver_cands_time.append(metrics[8])
#     print('The largest overlap found out is ', metrics[12])
#     print()

# df_results = pd.DataFrame({'r_id': r_id_list, 's_id': s_id_list, 
#                            'seeds': num_seeds_list, 'algo': algo_list, 'a%': a_perc_list, 'time': time_list,
#                            'gen_cands': num_gen_cands, 'gen_time': gen_cands_time, 
#                            'ver_cands': num_ver_cands, 'ver_time': ver_cands_time, 'flag': flag_list})
# df_results.to_csv(f'final_results_{dataset}/test_final_{constraint}_{dataset}_{n_samples}samples.csv', sep=';', index=False)
