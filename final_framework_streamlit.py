...
def sloth_mapping_based(r_tab, s_tab, min_a=0, min_w=0, max_w=math.inf, min_h=0, max_h=math.inf, bw=var.default_bw, complete=False,
    ...
    if row_constraint == 'none':
        results = [([seeds[s_id][0] for s_id in res],
                list(compute_bag_intersection(r_tab, s_tab, res, seeds).elements())) for res in results]
        results.append(pd.DataFrame.from_records(results[0][1]).sort_values(by=0, axis="index", ignore_index=True))
        return results, metrics

    elif row_constraint == 'ordered':
        _, idx_list = compute_long_comm_subseq(r_tab, s_tab, results[0], seeds, index=True)
        results.append(pd.DataFrame(dati_dict).iloc[idx_list])

    else:
        h, start_idx = compute_long_comm_substr(r_tab, s_tab, results[0], seeds, index=True)
        end_idx = start_idx + h - 1
        results.append(pd.DataFrame(dati_dict).loc[start_idx:end_idx])
    return results, metrics
